...
| 분류 | 소유하는 설정과 기능 |
| --- | --- |
| 일반 | 다운로드 루트, 저장공간 현황, Explore 페이지 크기, 카드 미리보기 크기, 프라이버시 모드, 업데이트·초기화·앱 정보 |
| Hitomi | 앨범 폴더 템플릿, Auto Find 기록, 판본 자동 판정, 앨범 열 수, Related galleries, Hitomi 요청 속도, 태그 카탈로그·전역 검색 규칙·탐색 제외 복원·라이브러리 유지보수 |
| Danbooru | 기본 rating, 기본 파일 형식, 기본 정렬, post 검색 메타데이터 안내, large/sample 카드 품질 정책 |

일반 설정은 두 소스가 같은 값과 동작을 사용한다. 예를 들어 카드 미리보기 크기는
Hitomi 앨범 카드와 Danbooru post 카드 폭에 함께 적용되고, 프라이버시 모드는 두
소스의 카드와 플로팅 상세 이미지를 모두 가린다. Hitomi 또는 Danbooru 탭의 설정은
다른 소스의 검색 상태나 다운로드 판정에는 영향을 주지 않는다.
