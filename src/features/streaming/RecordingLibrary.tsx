...
import { hasCompletedMerge, RecordingPlayback, recordingMergeLabel } from "./RecordingPlayback";
...
const statuses = { recording: "녹화 중", stopped: "저장 완료", interrupted: "중단됨", failed: "실패" };
...
const needsAttention = (recording: BrowserRecording) => recording.deletionPending || recording.status === "failed" || recording.status === "interrupted" || recording.merge?.status === "failed" || recording.merge?.status === "blocked" || recording.merge?.sourceCleanup?.status === "blocked" || recordingChatNote(recording).warning;
...
export function RecordingLibrary({ recordings, selectedId, onSelect, disabled, privacy, retrying, opening, onFolder, onReplay, onOpenMerged, onRetryMerge, onOpenSegment, onDelete, stopControl }: Props) {
  ...
  const saved = recordings.filter(item => item.status !== "recording");
  const filtered = useMemo(() => saved.filter(item => (privacy || !query.trim() || `${item.title} ${item.channelId}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase()))
    && (filter === "all" || filter === "ready" && hasCompletedMerge(item) || filter === "attention" && needsAttention(item))), [recordings, query, filter, privacy]);
  ...
  const selected = [...active, ...filtered].find(item => item.id === selectedId) ?? filtered[0] ?? active[0];
  ...
  return <section className="recording-library" aria-label="녹화 보관함">
    ...
    {selected ? <section className="recording-library-detail official-browser-files" aria-label="공식 녹화 파일">
      ...
      <p className="official-browser-muted" title={recordingChatNote(selected).text}>{statuses[selected.status]} · {duration(selected.durationSeconds)} · {bytes(hasCompletedMerge(selected) ? selected.merge!.bytes! : selected.bytesWritten)} <ChatWarning recording={selected} /></p>
      {selected.lastError ? <p className="official-browser-error" role="alert">{selected.lastError}</p> : null}
