...
function fixture(inline: boolean, replaceHeader: boolean): string {
...
async function render(inline: boolean, replaceHeader = false): Promise<Result> {
  const root = fs.mkdtempSync(path.join(tmpdir(), "atsumi-player-geometry-"));
  try {
    const html = path.join(root, "fixture.html");
    fs.writeFileSync(html, fixture(inline, replaceHeader));
    ...
    if (!serialized) throw new Error(`Isolated player geometry returned no result: ${(/<output id="geometry-error">([^<]*)<\/output>/.exec(stdout)?.[1] ?? "no fixture result").slice(0, 300)}`);
