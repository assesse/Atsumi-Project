...
import { hiddenOfficialBrowserViewport, type OfficialBrowserViewport } from "../../api/officialBrowser";
import { hasNativeOverlay, measureOfficialBrowserViewport } from "./OfficialBrowserPanel";
...
function Pane({ pane, epoch, api, privacy, onError }: { pane: MultiviewPane; epoch: number; api: MultiviewApi; privacy: boolean; onError(message: string): void }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const viewport = { ...measured, occluded: measured.visible && hasNativeOverlay() };
    ...
    update();
    return () => {
      disposed = true; revision++; latest = null; clearTimeout(retry); cancelAnimationFrame(frame); resize?.disconnect(); mutation.disconnect();
...
export function MadoWorkspace({ runtime, privacy, onLeave, api: suppliedApi }: { runtime: MultiviewApi["runtime"]; privacy: boolean; onLeave(): void; api?: MultiviewApi }) {
  ...
  const control = api.runtime === "tauri" && requestedControl && controlPane && ["record_start", "record_stop", "screenshot"].includes(requestedControl.action)
  ...
  const canApprove = !!control && !pending && (control.action === "record_stop" ? isRecording(controlPane!) && controlPane!.recordingStatus !== "stopping"
    : rights && !pollError && controlPane!.ready === true && (control.action !== "record_start" || !isRecording(controlPane!)));
  useEffect(() => {
  ...
  useEffect(() => {
    if (control) { setSelectedPaneId(control.paneId); setRights(false); cancelControl.current?.focus(); }
  }, [control?.id]);
  useEffect(() => {
  ...
  const audio = (pane: MultiviewPane) => void run(async (isCurrent) => {
    const result = await api.setPaneAudio(pane.paneId, !pane.audioEnabled, snapshot.epoch);
    if (!isCurrent()) return;
  ...
  const confirm = (approve: boolean) => {
    ...
    void run(async (isCurrent) => {
      const result = await api.confirmControl({ paneId: intent.paneId, requestId: intent.id, approve, rightsAcknowledged: rights, captureChat, epoch: snapshot.epoch });
      if (!isCurrent()) return;
  ...
  useEffect(() => {
    const shortcut = (event: KeyboardEvent) => {
      ...
      const pane = videos.find((item) => item.paneId === selectedPaneId) ?? videos[0];
  ...
  }, [api, privacy, pending, pollError, control?.id, snapshot, selectedPaneId]);
  ...
  const renderPane = (pane: MultiviewPane) => <Pane key={pane.paneId} pane={pane} epoch={snapshot.epoch} api={api} privacy={privacy || resizing || !!control} onError={setError} />;
  const title = (channelId: string) => {
    const video = snapshot.panes.find((pane) => pane.channelId === channelId && pane.kind === "video");
    const chatIssue = chatWarning(video);
    return <div className="mado-pane-label">{video ? <button type="button" className="mado-pane-name" title="이 방송 선택 · S로 화면 저장 요청" aria-pressed={(selectedPaneId ?? snapshot.panes.find((pane) => pane.kind === "video")?.paneId) === video.paneId} onClick={() => setSelectedPaneId(video.paneId)}>{label(channelId)}</button> : <span>{label(channelId)}</span>}
      {video ? <button type="button" className="mado-audio" disabled={pending} aria-label={`${label(channelId)} 소리 ${video.audioEnabled ? "끄기" : "켜기"}`} aria-pressed={!!video.audioEnabled} onClick={() => audio(video)}>{video.audioEnabled ? "♪" : "♩"}</button> : null}
      {video && isRecording(video) ? <span className="mado-recording" role="status">{video.recordingStatus === "stopping" ? "저장 중" : "녹화 중"}</span> : null}
      {video?.error ? <span className="mado-pane-warning" role="alert" title={video.error}>확인 필요</span> : null}
      {chatIssue ? <span className="mado-pane-warning mado-chat-warning" role="alert" tabIndex={0} title={chatIssue} aria-label={chatIssue}>⚠</span> : null}
    </div>;
  };
  return <section className="mado-workspace" aria-label="마도 모드">
    <header className="mado-toolbar"><strong title="마법도서관 · 최대 네 방송의 영상과 채팅을 배치합니다">마도</strong>
      <button type="button" onClick={() => setSettings(!settings)} aria-expanded={settings}>배치</button>
      ...
      <button type="button" disabled={pending || activeRecording} title={activeRecording ? "녹화를 먼저 중지해 주세요." : undefined} onClick={leave}>한 방송 보기</button>
    </header>
    {settings ? <form className="mado-settings" onSubmit={(event) => { event.preventDefault(); apply(); }}>
      <div className="mado-layout-options"><button type="button" aria-pressed={mode === "paired"} onClick={() => { edited.current = true; setMode("paired"); }}>방송·채팅</button><button type="button" aria-pressed={mode === "chats"} onClick={() => { edited.current = true; setMode("chats"); }}>영상 하나·채팅 모아보기</button></div>
      {inputs.map((input, index) => <div className="mado-channel-input" key={index}><label htmlFor={`mado-channel-${index}`}>방송 {index + 1}</label><input id={`mado-channel-${index}`} value={input} disabled={pending} autoComplete="off" spellCheck={false} placeholder="채널 주소 또는 ID" onChange={(event) => { edited.current = true; setInputs((rows) => rows.map((row, i) => i === index ? event.target.value : row)); }} />{mode === "chats" ? <label><input type="radio" name="mado-lead" checked={lead === index} onChange={() => { edited.current = true; setLead(index); }} />영상</label> : null}</div>)}
      <div className="mado-apply"><button type="submit" disabled={pending || activeRecording || runtime !== "tauri"}>{pending ? "연결 중…" : "적용"}</button><small title="고화질 연결과 로그인은 한 방송 보기에서 먼저 준비해 주세요. 녹화 중에는 채널 배치를 바꿀 수 없습니다.">최대 4개 <span aria-hidden="true">ⓘ</span></small></div>
    </form> : null}
    ...
    {snapshot.active ? <div ref={surfaces} className={`mado-surfaces is-${appliedMode} direction-${layout.direction}${resizing ? " is-resizing" : ""}`} data-channels={channels.length} style={{ "--mado-main-share": `${layout[layout.direction]}fr`, "--mado-chat-share": `${100 - layout[layout.direction]}fr` } as CSSProperties}>
      {appliedMode === "paired" ? channels.map((channelId) => <div className="mado-pair" key={channelId}>{title(channelId)}{snapshot.panes.filter((pane) => pane.channelId === channelId).map(renderPane)}</div>) : <>
        <div className="mado-lead">{snapshot.panes.filter((pane) => pane.kind === "video").map((pane) => <div className="mado-lead-inner" key={pane.paneId}>{title(pane.channelId)}{renderPane(pane)}</div>)}</div>
        ...
        <div className="mado-chat-wall">{snapshot.panes.filter((pane) => pane.kind === "chat").map((pane) => <div className="mado-chat-cell" key={pane.paneId}><span className="mado-pane-label">{label(pane.channelId)}</span>{renderPane(pane)}</div>)}</div>
      </>}
    </div> : <p className="mado-empty">방송 주소를 입력하고 원하는 배치를 적용해 주세요.</p>}
    {control ? <div className="official-browser-dialog-backdrop" role="alertdialog" aria-modal="true" aria-labelledby="mado-control-title" onKeyDown={controlKeys}>
      <div className="official-browser-dialog"><h3 id="mado-control-title">{control.action === "record_start" ? "녹화를 시작할까요?" : control.action === "record_stop" ? "녹화를 중지할까요?" : "화면을 저장할까요?"}</h3><p>{label(control.channelId)} · 플레이어에서 요청한 동작입니다.</p>
        {control.action !== "record_stop" ? <label className="official-browser-control-rights"><input type="checkbox" checked={rights} disabled={pending} onChange={(event) => setRights(event.target.checked)} /> 이 방송을 저장할 권한과 적용되는 이용 조건을 확인했습니다.</label> : null}
        {control.action === "record_start" ? <><label className="official-browser-control-rights"><input type="checkbox" checked={captureChat} disabled={pending} onChange={(event) => setCaptureChat(event.target.checked)} /> 채팅도 저장</label><p>탐색·배속·채널 변경 시 녹화가 중단됩니다.</p></> : null}
        {error || pollError ? <p role="alert" className="official-browser-error">{error || pollError}</p> : null}
        <div><button ref={cancelControl} type="button" disabled={pending} onClick={() => confirm(false)}>취소</button><button type="button" disabled={!canApprove} onClick={() => confirm(true)}>{pending ? "처리 중…" : control.action === "record_start" ? "시작 확인" : control.action === "record_stop" ? "중지 확인" : "저장 확인"}</button></div>
      </div>
    </div> : null}
  </section>;
}
