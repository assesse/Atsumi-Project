...
async function snapshots(): Promise<Frame[]> {
  ...
  try {
    for (const name of ["ready", "recording", "stopping", "error", "screenshot", "settings", "setup"]) {
      ...
      const api: OfficialBrowserApi = { runtime: "tauri", snapshot, open: snapshot, start: snapshot, stop: snapshot, login: snapshot, logout: snapshot, connectExtension: snapshot, confirmControl: snapshot, requestControl: snapshot, ackUiAction: snapshot, setViewport: nothing, openInstaller: nothing, openFolder: nothing, openSegment: nothing, openMerged: nothing, retryMerge: snapshot };
