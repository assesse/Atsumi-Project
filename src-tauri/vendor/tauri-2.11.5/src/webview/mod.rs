...
```
  ...
  pub fn on_message(self, request: InvokeRequest, responder: Box<OwnedInvokeResponder<R>>) {
    ...
    // Atsumi policy: remote pages use a separate, narrowly scoped native bridge.
    // Reject before ACL/plugin dispatch, including the internal channel-fetch
    // exception below. Both HTTP IPC and WebMessage IPC pass through this gate.
    if !is_local {
      resolver.reject("Tauri IPC is disabled for remote content");
      return;
    }
...
mod tests {
  ...
  fn atsumi_ipc_request(command: &str, origin: &str) -> super::InvokeRequest {
    super::InvokeRequest {
      cmd: command.into(),
      callback: crate::ipc::CallbackFn(0),
      error: crate::ipc::CallbackFn(1),
  ...
  #[test]
  fn atsumi_remote_ipc_is_rejected_for_every_window_label() {
    use crate::test::{get_ipc_response, mock_builder, mock_context, noop_assets};

    let app = mock_builder()
      .invoke_handler(|_| panic!("remote IPC must not reach the app handler"))
      .build(mock_context(noop_assets()))
      .unwrap();

    for label in ["main", "chzzk-official-browser", "chzzk-login-test"] {
      let webview = crate::WebviewWindowBuilder::new(&app, label, Default::default())
        .build()
        .unwrap();
      for origin in [
        ...
        "http://127.0.0.1:1420/",
        "data:text/html,test",
        "blob:https://chzzk.naver.com/test",
      ] {
        for command in [
          "atsumi_test_ping",
          ...
          let result = get_ipc_response(&webview, atsumi_ipc_request(command, origin));
          assert_eq!(
            result.err(),
            Some(serde_json::json!(
              "Tauri IPC is disabled for remote content"
            )),
            "remote request must be rejected: {label}, {origin}, {command}"
          );
        }
      }
    }
  ...
  #[test]
  fn atsumi_remote_capability_cannot_override_native_ipc_policy() {
    use crate::test::{get_ipc_response, mock_builder, mock_context, noop_assets};
    use crate::utils::acl::ExecutionContext;
    ...
    let app = mock_builder()
      .invoke_handler(|_| panic!("remote capability must not bypass the native policy"))
      .build(context)
      .unwrap();
    let webview = crate::WebviewWindowBuilder::new(&app, "main", Default::default())
      .build()
    ...
    for command in [
      ...
      assert_eq!(
        get_ipc_response(
          &webview,
          atsumi_ipc_request(command, "https://chzzk.naver.com/live/test")
        )
        .err(),
        Some(serde_json::json!(
          "Tauri IPC is disabled for remote content"
  ...
  #[test]
  fn atsumi_local_custom_ipc_and_internal_channel_dispatch_are_preserved() {
    use crate::test::{get_ipc_response, mock_builder, mock_context, noop_assets};

    let app = mock_builder()
      .invoke_handler(|invoke| {
        assert_eq!(invoke.message.command(), "atsumi_test_ping");
        invoke.resolver.resolve("pong".repeat(4096));
        true
      })
      .build(mock_context(noop_assets()))
      .unwrap();
    ...
    let response = get_ipc_response(&webview, atsumi_ipc_request("atsumi_test_ping", origin))
      .unwrap()
      .deserialize::<String>()
      .unwrap();
    assert_eq!(response, "pong".repeat(4096));
    // No data ID is requested: this checks routing into the actual channel
    // plugin without reading or interfering with any other test's queue entry.
    assert_eq!(
      get_ipc_response(
        &webview,
        atsumi_ipc_request(crate::ipc::channel::FETCH_CHANNEL_DATA_COMMAND, origin)
      )
      .err(),
      Some(serde_json::json!("missing channel id header"))
    );
  }

  #[cfg(dev)]
  #[test]
  fn atsumi_configured_dev_origin_remains_local_but_other_origins_do_not() {
    use crate::test::{get_ipc_response, mock_builder, mock_context, noop_assets};
    ...
    assert_eq!(
      get_ipc_response(
        &webview,
        atsumi_ipc_request("atsumi_test_ping", "http://127.0.0.1:1420/nested/path")
      )
      .unwrap()
      .deserialize::<String>()
      .unwrap(),
    ...
    for origin in [
      ...
      assert_eq!(
        get_ipc_response(&webview, atsumi_ipc_request("atsumi_test_ping", origin)).err(),
        Some(serde_json::json!(
          "Tauri IPC is disabled for remote content"
        ))
      );
