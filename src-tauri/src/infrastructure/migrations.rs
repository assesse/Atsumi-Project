...
pub const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        version: 40,
        name: "persistent_gallery_and_artist_previews",
        sql: r#"
            CREATE TABLE gallery_previews (
                gallery_id INTEGER PRIMARY KEY REFERENCES galleries(gallery_id) ON DELETE CASCADE,
                mode TEXT NOT NULL DEFAULT 'automatic' CHECK (mode IN ('automatic', 'manual')),
                manual_source_page INTEGER CHECK (manual_source_page IS NULL OR manual_source_page > 0),
                entry_id TEXT REFERENCES download_entries(entry_id) ON DELETE SET NULL,
                candidates_json TEXT NOT NULL DEFAULT '[]' CHECK (json_valid(candidates_json)),
                algorithm_version INTEGER NOT NULL DEFAULT 0 CHECK (algorithm_version >= 0),
                analysis_attempted_at TEXT,
                updated_at TEXT NOT NULL,
                CHECK ((mode = 'automatic' AND manual_source_page IS NULL) OR (mode = 'manual' AND manual_source_page IS NOT NULL))
            ) STRICT;

            CREATE TABLE artist_previews (
                artist TEXT PRIMARY KEY COLLATE NOCASE,
                gallery_ids_json TEXT NOT NULL CHECK (json_valid(gallery_ids_json)),
                updated_at TEXT NOT NULL
            ) STRICT;

            INSERT INTO artist_previews (artist, gallery_ids_json, updated_at)
            SELECT artist, json_group_array(gallery_id), strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
            FROM (
                SELECT artist, gallery_id FROM (
                    SELECT owned.artist, entry.gallery_id,
                           row_number() OVER (PARTITION BY owned.artist ORDER BY max(entry.created_at) DESC, entry.gallery_id DESC) AS rank
                    FROM owned_gallery_artists owned
                    JOIN download_entries entry ON entry.gallery_id = owned.gallery_id
                    JOIN download_artifacts artifact ON artifact.entry_id = entry.entry_id
                    WHERE entry.state = 'completed' AND artifact.state = 'complete'
                      AND NOT EXISTS (SELECT 1 FROM duplicate_hidden_galleries hidden WHERE hidden.gallery_id = entry.gallery_id)
                    GROUP BY owned.artist, entry.gallery_id
                ) WHERE rank <= 5 ORDER BY artist, rank
            ) GROUP BY artist;
        "#,
    },
    Migration {
        version: 41,
        name: "recoverable_excluded_album_folders",
        sql: super::excluded_artifacts::EXCLUDED_ARTIFACTS_SCHEMA,
    },
    Migration {
        version: 42,
        name: "persistent_gallery_summaries",
        sql: super::gallery_summary_cache::GALLERY_SUMMARY_CACHE_SCHEMA,
    },
    Migration {
        version: 43,
        name: "repair_deployed_excluded_folder_path_guard",
        sql: super::excluded_artifacts::REPAIR_EXCLUDED_ARTIFACT_TRIGGER,
    },
    Migration {
        version: 44,
        name: "recoverable_overlap_page_merges",
        sql: super::overlap_merge::OVERLAP_MERGE_SCHEMA,
    },
    Migration {
        version: 45,
        name: "adaptive_download_concurrency",
        sql: r#"
            ALTER TABLE settings ADD COLUMN download_adaptive_concurrency INTEGER NOT NULL
                DEFAULT 1 CHECK (download_adaptive_concurrency IN (0, 1));
            ALTER TABLE settings ADD COLUMN download_adaptive_max_requests INTEGER NOT NULL
                DEFAULT 8 CHECK (download_adaptive_max_requests BETWEEN 1 AND 8);

            CREATE TABLE IF NOT EXISTS download_tuning_profiles (
                host TEXT PRIMARY KEY NOT NULL,
                algorithm_version INTEGER NOT NULL,
                stable_limit INTEGER NOT NULL,
                baseline_bytes_per_second REAL NOT NULL,
                updated_at_unix_ms INTEGER NOT NULL,
                cooldown_until_unix_ms INTEGER NOT NULL,
                blocked_until_unix_ms INTEGER NOT NULL
            );
            CREATE INDEX IF NOT EXISTS idx_download_tuning_profiles_recent
                ON download_tuning_profiles(updated_at_unix_ms DESC, host ASC);
        "#,
    },
    Migration {
        version: 46,
        name: "persist_gallery_participating_artists",
        sql: r#"
            ALTER TABLE auto_find_candidates ADD COLUMN artists_json TEXT NOT NULL
                DEFAULT '[]' CHECK (json_valid(artists_json) AND json_type(artists_json) = 'array');

            -- Recover only known local participation metadata. A matching
            -- favorite is not evidence for a complete list or an extra count.
            UPDATE auto_find_candidates AS candidate
            SET artists_json = (
                SELECT json_extract(summary.summary_json, '$.artists')
                FROM gallery_summary_cache summary
                WHERE summary.gallery_id = candidate.gallery_id
                  AND summary.profile = 'hitomi-gallery-summary-v1'
                  AND summary.schema_version = 1
                  AND CASE WHEN json_valid(summary.summary_json) THEN
                      json_extract(summary.summary_json, '$.id') = candidate.gallery_id
                      AND json_type(summary.summary_json, '$.artists') = 'array'
                      AND json_array_length(summary.summary_json, '$.artists') > 0
                      AND NOT EXISTS (
                          SELECT 1 FROM json_each(summary.summary_json, '$.artists') artist
                          WHERE artist.type != 'text'
                      )
                  ELSE 0 END
            )
            WHERE EXISTS (
                SELECT 1 FROM gallery_summary_cache summary
                WHERE summary.gallery_id = candidate.gallery_id
                  AND summary.profile = 'hitomi-gallery-summary-v1'
                  AND summary.schema_version = 1
                  AND CASE WHEN json_valid(summary.summary_json) THEN
            ...
            SET artists_json = (
                SELECT json_group_array(artist) FROM (
                    ...
                    WHERE owned.gallery_id = candidate.gallery_id
                    ORDER BY artist COLLATE NOCASE ASC
                )
            )
            WHERE artists_json = '[]'
              AND EXISTS (
                  SELECT 1 FROM owned_gallery_artists owned
                  WHERE owned.gallery_id = candidate.gallery_id
              );
        "#,
    },
...
mod tests {
    ...
    fn participating_artist_migration_backfills_only_valid_known_local_lists() {
        ...
        connection
            ...
            INSERT INTO auto_find_runs (
                run_id, revision, state, total_favorites, completed_favorites,
                candidates_found, started_at, updated_at
            ) VALUES ('legacy-artists', 1, 'completed', 1, 1, 7, 'now', 'now');
        "#,
            )
            .unwrap();
        for id in 1..=7 {
            connection
                ...
                INSERT INTO auto_find_candidates (
                    run_id, gallery_id, title, artist, pages, language, tags_json,
                    published_rank, popularity, thumbnail_width, thumbnail_height,
                    favorite_namespace, favorite_value, discovered_at
                ) VALUES ('legacy-artists', ?1, 'Album', 'original first', 10, 'korean',
                          '[]', 1, 1, 100, 150, 'artist', 'unproven matching name', 'now')
            "#,
                    [id],
                )
                .unwrap();
        }
        for (id, profile, version, json) in [
            (
                1,
                "hitomi-gallery-summary-v1",
                1,
                r#"{"id":1,"artists":["original first","collaborator"]}"#,
            ),
            (
                2,
                "hitomi-gallery-summary-v1",
                1,
                r#"{"id":2,"artist":"original first"}"#,
            ),
            (3, "hitomi-gallery-summary-v1", 1, "{broken"),
            (
                4,
                "hitomi-gallery-summary-v1",
                1,
                r#"{"id":4,"artists":["name",42]}"#,
            ),
            (
                5,
                "obsolete-profile",
                1,
                r#"{"id":5,"artists":["wrong profile"]}"#,
            ),
            (
                6,
                "hitomi-gallery-summary-v1",
                2,
                r#"{"id":6,"artists":["wrong version"]}"#,
            ),
            (
                7,
                "hitomi-gallery-summary-v1",
                1,
                r#"{"id":8,"artists":["wrong id"]}"#,
            ),
        ] {
            connection.execute(
                "INSERT INTO gallery_summary_cache (gallery_id, profile, schema_version, summary_json) VALUES (?1, ?2, ?3, ?4)",
                params![id, profile, version, json],
            ).unwrap();
        }
        connection
            .execute_batch(
                r#"
            INSERT INTO galleries (gallery_id, revision, title, primary_artist, source_page_count)
                VALUES (2, 0, 'Owned collaboration', 'original first', 10);
            INSERT INTO owned_gallery_artists (gallery_id, artist) VALUES
                (2, 'original first'), (2, 'owned collaborator');
        "#,
            )
            .unwrap();
        ...
        for id in 1..=7 {
            let (first, json): (String, String) = connection
                .query_row(
                    "SELECT artist, artists_json FROM auto_find_candidates WHERE gallery_id = ?1",
                    [id],
                    |row| Ok((row.get(0)?, row.get(1)?)),
                )
                .unwrap();
            assert_eq!(first, "original first");
            let names: Vec<String> = serde_json::from_str(&json).unwrap();
            let expected = match id {
                1 => vec!["original first", "collaborator"],
                2 => vec!["original first", "owned collaborator"],
                _ => vec![],
            };
            assert_eq!(names, expected, "gallery {id}");
        }
    ...
    #[test]
    fn adaptive_download_migration_preserves_explicit_request_settings() {
        let mut connection = migration_history(&[]);
        for migration in MIGRATIONS
            .iter()
        ...
        connection.execute(
            "UPDATE settings SET revision = 12, concurrent_image_requests = 2, request_start_interval_ms = 375 WHERE singleton = 1",
            [],
        ...
        ).expect("read upgraded settings");
        assert_eq!(settings, (12, 2, 375, true, 8));
        let profile_count: i64 = connection
            .query_row("SELECT COUNT(*) FROM download_tuning_profiles", [], |row| {
                row.get(0)
            })
            .expect("profile storage is created empty");
        assert_eq!(profile_count, 0);
        for limit in [0, 9] {
            assert!(connection
                .execute(
                    "UPDATE settings SET download_adaptive_max_requests = ?1 WHERE singleton = 1",
                    [limit],
                )
                .is_err());
        }
