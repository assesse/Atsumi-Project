...
let offset = 0;
const ranges = new URLSearchParams(window.location.search).get("ranges") === "1";
const api: ReplayApi = {
  open: async () => ({ ok: true, data: { token: "synthetic-only", recordingId: "synthetic-only", title: "늦여름, 조용한 오후의 이야기 · 합성 미리보기", recordedAt: 1789272000000, durationSeconds: ranges ? 60 : 30, mimeType: "video/mp4", chatStatus: "stopped", indexState: "ready", syncQuality: "receive_time_approximate", manualOffsetSeconds: offset, warnings: [], ...(ranges ? { recordingActive: true, parts: [{ index: 0, startSeconds: 0, durationSeconds: 30 }, { index: 1, startSeconds: 30, durationSeconds: 30 }] } : {}) } }),
  ...
  mediaUrl: () => ranges ? "/.runtime/progressive-preview" : new URLSearchParams(window.location.search).get("synthetic") === "1" ? "/.runtime/chzzk-original-player/synthetic.mp4" : "",
};
