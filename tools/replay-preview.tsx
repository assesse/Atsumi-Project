...
let offset = 0;
const api: ReplayApi = {
  open: async () => ({ ok: true, data: { token: "synthetic-only", recordingId: "synthetic-only", title: "늦여름, 조용한 오후의 이야기 · 저장본 미리보기", durationSeconds: 3600, mimeType: "video/mp4", chatStatus: "stopped", indexState: "ready", syncQuality: "receive_time_approximate", manualOffsetSeconds: offset, warnings: [] } }),
  ...
  timeline: async () => ({ ok: true, data: { bucketSeconds: 30, indexState: "ready", viewerMetricStatus: "not_recorded", buckets: Array.from({ length: 120 }, (_, index) => ({ startSeconds: index * 30, chatCount: Math.round(20 + 65 * Math.sin(index * .18) ** 2 + 100 * Math.exp(-(((index - 77) / 8) ** 2))), uniqueSenderCount: null, viewerCount: null })) } }),
  ...
  mediaUrl: () => new URLSearchParams(window.location.search).get("synthetic") === "1" ? "/synthetic-replay-fixture.mp4" : "",
};
